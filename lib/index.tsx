...
type HyperRendererWindow = Window &
  typeof globalThis & {
    __hyperRendererBootstrap?: boolean;
...
const rendererWindow = window as HyperRendererWindow;

function defineWindowGetter(name: 'store' | 'rpc' | 'config' | 'plugins', getter: () => unknown) {
  if (Object.getOwnPropertyDescriptor(rendererWindow, name)) {
    return;
  }

  Object.defineProperty(rendererWindow, name, {
    configurable: true,
    get: getter
  });
}

if (!rendererWindow.__hyperRendererBootstrap) {
  rendererWindow.__hyperRendererBootstrap = true;

  // Avoid double-initializing the renderer when the entry bundle is evaluated twice.
  // This can happen during development reload flows or if the script is injected twice.
  ...
  defineWindowGetter('store', () => store_);
  defineWindowGetter('rpc', () => rpc);
  defineWindowGetter('config', () => config);
  defineWindowGetter('plugins', () => plugins);
