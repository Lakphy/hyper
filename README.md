...
Regardless of the platform you are working on, you will need to have pnpm installed. If you have never installed pnpm before, you can find out how at: https://pnpm.io/installation.

1. Install necessary packages:
  * Windows
    - Be sure to run `pnpm add -g windows-build-tools` from an elevated prompt (as an administrator) to install `windows-build-tools`.
  * macOS
    - Once you have installed pnpm, you can skip this section!
...
5. To run `hyper`
  ...
  * `pnpm run dev` uses Vite for the renderer bundle, TypeScript watch mode for the TypeScript projects, and a watcher that syncs `app/` static files into `target/`. If you interrupt it, you'll need to relaunch it before testing again. You'll just have to relaunch Electron by using `pnpm run app` or the VSCode launch task.
